...
<section class="section">
    <div class="section-header">
        <h1>Type de cotisation </h1>
    </div>
    <div class="card">
        <div class="row">
            <div class="col-6">
                <div class="card-header">
                    <h4>Liste des type de cotisation</h4>
                </div>
            </div>
        ...
        <div class="table-responsive">
            <table class="table table-striped">
                ...
                <tbody>
                    ...
                    @foreach ($natureContribution as $item)
                    <tr>
...
<script>
    ...
    $("#create_form").fireModal({
        ...
        onFormSubmit: function(modal, e, form) {
            ...
            url = "{{route('nature_contribution.create')}}"
            $.ajax({
                url: url,
                cache: false,
                contentType: false,
    ...
    natureContribution = @json($natureContribution);
    for (var i = 0; i < natureContribution.length; i < i++) {
        ...
        hideInput.value = natureContribution[i].id
        ...
        $("#delete" + natureContribution[i].id).fireModal({
            ...
            onFormSubmit: function(modal, e, form) {
                ...
                var url = '{{ route("nature_contribution.delete", ":id") }}';
        ...
        input.setAttribute("id", "libelle");
        input.setAttribute("name", "libelle");
        input.setAttribute("value", natureContribution[i].libelle);
        input.setAttribute("required", true);
        input.classList.add('form-control')


        var hideInput = document.createElement("input");
        hideInput.setAttribute("name", "natureContribution");
        hideInput.value = natureContribution[i].id
        hideInput.style.visibility = "hidden";
        ...
        $("#update" + natureContribution[i].id).fireModal({
            title: 'Modifier ',
            body: $(form),
            footerClass: 'bg-whitesmoke',
            autoFocus: false,
            onFormSubmit: function(modal, e, form) {
                ...
                url = "{{route('nature_contribution.update')}}"
                $.ajax({
                    url: url,
                    cache: false,
                    contentType: false,
